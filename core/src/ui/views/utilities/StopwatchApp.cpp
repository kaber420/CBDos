...
void StopwatchApp::swToggleCb(lv_event_t* e) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        if (s_swRunning) {
            ...
            if (s_swStartLbl && lv_obj_is_valid(s_swStartLbl)) lv_label_set_text(s_swStartLbl, "Pausar");
            if (s_swStartBtn && lv_obj_is_valid(s_swStartBtn)) lv_obj_set_style_bg_color(s_swStartBtn, lv_color_hex(0xFF2E93), 0);
        } else {
            if (s_swStartLbl && lv_obj_is_valid(s_swStartLbl)) lv_label_set_text(s_swStartLbl, "Iniciar");
            if (s_swStartBtn && lv_obj_is_valid(s_swStartBtn)) lv_obj_set_style_bg_color(s_swStartBtn, DefaultTheme::getPrimaryAccent(), 0);
        }
    }
}
