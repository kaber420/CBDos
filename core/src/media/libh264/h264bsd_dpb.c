...
/* Code */
    ...
    if (dpb->outIndex < dpb->numOut)
    {
        ...
        /* When buffer is drained, reset counters so next frame can be stored
         * (required for noReordering mode where numOut/outIndex are never
         * reset by the DPB internals between frames). */
        if (dpb->outIndex >= dpb->numOut)
        {
            dpb->numOut   = 0;
            dpb->outIndex = 0;
