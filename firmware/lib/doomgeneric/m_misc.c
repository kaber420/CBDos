...
boolean M_FileExists(char *filename)
{
    ...
    // Probar con versión en mayúsculas (común en tarjetas FAT32 SD)
    char altname[256];
    strncpy(altname, filename, sizeof(altname) - 1);
    altname[sizeof(altname) - 1] = '\0';
    for (int i = 0; altname[i]; i++) altname[i] = toupper((unsigned char)altname[i]);
    
    fstream = fopen(altname, "rb");
    if (fstream != NULL)
    {
        fclose(fstream);
        return true;
