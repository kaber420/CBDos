...
#ifdef ARDUINO
#define DEFAULT_RAM 5 /* MiB — 100% libre para el cartucho en 8MB PSRAM */
...
#ifdef ARDUINO
        zonemem = (byte*)heap_caps_malloc(*size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
        zonemem = malloc(*size);
#endif
...
void I_Error (char *error, ...)
{
...
#ifdef ARDUINO
    extern void Doom_ReportError(const char* msg);
    Doom_ReportError(msgbuf);
#endif
    ...
    exit(-1);
#elif defined(ARDUINO)
    // ESP32: no exit(), signal error via serial and stall
    printf("[DOOM] I_Error: fatal, stalling task\n");
