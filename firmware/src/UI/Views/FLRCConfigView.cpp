#include "FLRCConfigView.h"
#include "../UIManager.h"
#include "../Themes/DefaultTheme.h"
#include <cstdio>
#include <cstdlib>

HeaderBar* FLRCConfigView::headerBar = nullptr;
FLRCConfig FLRCConfigView::currentCfg;
lv_obj_t* FLRCConfigView::taFreq = nullptr;
lv_obj_t* FLRCConfigView::taPwr = nullptr;
lv_obj_t* FLRCConfigView::taBw = nullptr;
lv_obj_t* FLRCConfigView::taRate = nullptr;

void FLRCConfigView::save_event_cb(lv_event_t* e) {
    ...
    if (taRate) currentCfg.dataRate = atoi(lv_textarea_get_text(taRate));

    if (ConfigManager::getInstance().saveFLRC(currentCfg)) {
        UIManager::showToast("FLRC guardado correctamente");
        UIManager::getInstance().loadConfigView();
    } else {
        UIManager::showToast("Error al guardar FLRC");
    }
}

lv_obj_t* FLRCConfigView::create() {
    ConfigManager::getInstance().loadFLRC(currentCfg);
    ...
    headerBar = HeaderBar::create(screen, "FLRC (2.4GHz)", true, true);
    ...
    lv_label_set_text(lblFreq, "Frecuencia (GHz):");
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "%.3f", currentCfg.frequency);
    ...
    lv_label_set_text(lblBw, "Ancho de Banda (MHz):");
    ...
    // Data Rate
    lv_obj_t* lblRate = lv_label_create(content);
    lv_label_set_text(lblRate, "Data Rate (0=1Mbps, 1=1.3Mbps):");
    lv_obj_set_style_text_color(lblRate, DefaultTheme::getTextColor(), 0);

    taRate = lv_textarea_create(content);
    lv_obj_set_width(taRate, lv_pct(100));
    lv_textarea_set_one_line(taRate, true);
    snprintf(buf, sizeof(buf), "%d", currentCfg.dataRate);
    lv_textarea_set_text(taRate, buf);
    DefaultTheme::applyRaisedCard(taRate, 10);
    UIManager::attachKeyboard(taRate);
    ...
    lv_label_set_text(lblBtn, "Guardar FLRC");
