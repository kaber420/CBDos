...
void PomodoroApp::triggerAlert() {
    if (s_soundType != PomodoroSound::MUTE) {
        NativeAudioDriver::getInstance().playTonePattern((int)s_soundType);
    }
}
...
void PomodoroApp::updateUI() {
    ...
    if (s_cycleLbl && lv_obj_is_valid(s_cycleLbl)) {
        lv_label_set_text_fmt(s_cycleLbl, "Sesión %d/4", s_cycleCount);
    }
    ...
    for (int i = 0; i < 4; i++) {
        if (s_presetBtns[i] && lv_obj_is_valid(s_presetBtns[i])) {
            ...
            lv_obj_set_state(s_presetBtns[i], LV_STATE_CHECKED, active);
        }
    }
    ...
    if (s_customRow && lv_obj_is_valid(s_customRow)) {
        if (s_selectedMode == PomodoroMode::CUSTOM) {
            lv_obj_clear_flag(s_customRow, LV_OBJ_FLAG_HIDDEN);
    ...
    for (int i = 0; i < 4; i++) {
        if (s_soundBtns[i] && lv_obj_is_valid(s_soundBtns[i])) {
            ...
            lv_obj_set_state(s_soundBtns[i], LV_STATE_CHECKED, active);
        }
    }
}
...
void PomodoroApp::build(lv_obj_t* parent) {
    ...
    lv_label_set_text(s_cycleLbl, "Sesión 1/4");
    ...
    lv_obj_set_style_text_font(s_timeLbl, &lv_font_montserrat_32, 0);
    ...
    lv_label_set_text(nextLbl, "Saltar >>");
